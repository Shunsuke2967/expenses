...
RSpec.describe 'TemplatesControllers', type: :request do
  ...
  describe 'POST #create' do
    subject { post templates_path, params: }
    let(:params) do
      {
        template: {
          icon: 'rent',
  ...
  describe 'PATCH #update' do
    let!(:template) { create(:template, memo: 'テスト用テンプレートメモ', user:) }
    subject { patch template_path(template), params: }
  ...
  describe 'GET #day_select' do
    let(:template) { create(:template, memo: 'テンプレートメモ1', user:) }
    let(:template_1) { create(:template, memo: 'テンプレートメモ2', user:) }
    let(:template_2) { create(:template, memo: 'テンプレートメモ3', user:) }
    let(:params) { { ids: [template.id, template_1.id, template_2.id] } }
    before { get day_select_templates_path, params: }
  ...
  describe 'POST #days_create' do
    ...
    subject { post days_create_templates_path, params: }
